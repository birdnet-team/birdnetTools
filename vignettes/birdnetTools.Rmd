...
birdnet_subsample(
  ...
  file = "swainsons_thrush_validation.csv" # name of the file to save the subsample
)
```
...
```{r}
# simulate validation column based on confidence
set.seed(123)  # for reproducibility
data_validated <- data_subsampled %>%
  mutate(
    validation = case_when(
      confidence >= 0.7 ~ 1,
      confidence >= 0.4 & confidence < 0.7 ~ sample(c(1, 0), n(), replace = TRUE, prob = c(0.8, 0.2)),
      TRUE ~ sample(c(1, 0), n(), replace = TRUE, prob = c(0.3, 0.7))
    )
  )

data_validated <- data_subsampled %>%
  mutate(
    validation = rbinom(n(), size = 1, prob = pmin(pmax(confidence, 0), 1))
  )
```
...
```{r}
# get the threshold
birdnet_calc_threshold(data_validated,
                       precision = 0.9)
