...
Continuing with our filtered data, `data_filtered` contains detections of "Swainson's Thrush" with a confidence threshold of 0.2. To validate these detections, we want to subsample a portion of them for manual verification. Specifically, the user can select a number of detections to validate, confirming whether they indeed contain Swainson's Thrush.

The `birdnet_subsample() `function offers three types of subsampling methods:

- Stratified sampling: selects detections evenly across the range of confidence scores, ensuring the validation set represents the distribution of confidence levels.

- Random sampling: selects detections at random, providing a general unbiased subset.

- Top sampling: selects detections with the highest confidence scores, focusing on the most confident detections.

Stratified and random sampling are useful when aiming for a validation set that is representative of the entire dataset, while top sampling is beneficial if the focus is on validating the most confident detections.
...
Optionally, the user can also save the file, as csv, to the local working directory. 
...
birdnet_subsample(
  ...
  file = "swainson's_thrush_validation.csv" # name of the file to save the subsample
)
```
...
The next step in the validation process is to listen to the audio files or observe the signals in the spectrogram to confirm detections. This task can be tedious and very time-consuming, as it requires manually clicking through each audio file, finding the exact timestamp, and listening to detections one by one.

The `birdnet_validation_app()` function provides a Shiny app that simplifies this process by offering an interactive interface. By supplying the subsampled detection data along with the folder containing your audio files, the app displays the audio clips and their corresponding detections, allowing users to easily listen to each detection and confirm whether it is correct.

```{r eval=FALSE}
birdnet_launch_validation()
```

1. After running the above code, an interface should pop up if all goes well. This interface required two entries:

- The .csv file that contains the detections. This should be a .csv file containing selected BirdNET predictions for validation
...
2. The .csv file would be shown on the top panel (with few selected columns). Once you click on the “Spectrogram” button, the bottom panel would show the corresponded spectrogram. And if you click the “Audio” button, it would play that specific segment for you.

3. You can adjust the “Settings” on the left panel to view the spectrogram. Click on the “Praise me” button if you are feeling tired of validating sounds. 
